...
; La definición del problema indica que hay un muro paralelo al
; eje Y, siempre que Y sea mayor que -1.
; Esto implica que si estamos en (2, -1) no podemos ir hacia arriba,
; si estamos en (1, Y) con Y > -1 no podemos ir a la derecha,
; y si estamos en (3, Y) con Y > -1 no podemos ir a la izq.
; En cualquier otro caso, podemos movernos libremente.

(deffunction aplicablep (?op $?coords)
	(bind ?x (nth 1 $?coords))
	(bind ?y (nth 2 $?coords))
	(if (and (eq ?x 2) (eq ?y -1) (eq ?op arr)) then
		FALSE
	else
		(if (and (eq ?x 1) (> ?y -1) (eq ?op der)) then
			FALSE
		else
			(if (and (eq ?x 3) (> ?y -1) (eq ?op izq)) then
				FALSE
			else
				TRUE
			)
