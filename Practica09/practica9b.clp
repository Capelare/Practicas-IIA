...
(defrule seleccionar-jugada-max
...
(defrule solicitar-jugada-min
	...
	(not (recomendacion))
=>
	(printout t "Te toca, " min crlf)
	(printout t "La posicion es: " $?t crlf)
	(printout t "Que quieres hacer?" crlf)
	(bind ?x (readline))
	(assert-string(format nil "(recomendacion (operador %s))" ?x))
)
...
=>
	...
	(printout t "La nueva posicion es: " $?nt crlf)
	(assert
		(posicion
			(jugador min)
			(tablero $?nt)
...
(defrule jugada-de-min
	...
	?r <- (recomendacion (operador ?op))
=>
;	(printout t "Hola, soy " min crlf)
;	(printout t "La posicion es: " $?t crlf)
	(printout t "Voy a jugar: " ?op crlf)
	(bind $?nt (aplicar ?op $?t min))
	(printout t "La nueva posicion es: " $?nt crlf crlf)
	(assert (posicion (jugador max) (tablero $?nt)))
	(retract ?p ?r)
)
...
=>
	...
	(switch ?j
		(case min then (printout t "Ha ganado " max "!" crlf))
		(case max then (printout t "Ha ganado " min "!" crlf))
	)
	(retract ?p)
)